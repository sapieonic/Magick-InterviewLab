...
export async function saveQuestionAction(payload: unknown): Promise<ActionResult<SavedQuestion>> {
  return actionGuard(async () => {
    ...
    // Replace-in-transaction rather than diff: the client sends the intended
    // final list, and a partial apply would leave a question scored against
    // tests the admin thought they had deleted.
    const questionId = await prisma.$transaction(async (tx) => {
      ...
      if (testCaseRows.length > 0) {
        await tx.testCase.createMany({
          data: testCaseRows.map((row) => ({ ...row, questionId: id })),
        });
      }
